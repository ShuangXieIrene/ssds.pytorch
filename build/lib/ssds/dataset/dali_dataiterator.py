...
class DaliPipeline(object):
    def __init__(self, target_size, preproc_param, training=False):
        ...
        # ssd crop
        self.bbox_crop   = ops.RandomBBoxCrop(device='cpu', bbox_layout="xyXY", scaling=crop_scale, aspect_ratio=crop_aspect_ratio,
                                            allow_no_crop=True, thresholds=[0, 0.1, 0.3, 0.5, 0.7, 0.9], num_attempts=crop_attempts)

        # color twist
        self.uniform_con = ops.Uniform(range=contrast_range)
        self.uniform_bri = ops.Uniform(range=(1.0 - bri_delta/256.0, 1.0 + bri_delta/256.0))
        ...
        self.resize      = ops.Resize(device='gpu', interp_type=types.DALIInterpType.INTERP_CUBIC, resize_x=target_size[0], resize_y=target_size[1], save_attrs=True)
    ...
    def predefined_graph(self, images, bboxes, labels):
        if self.training:
            ...
            # color twist
            images = self.hsv(images, hue=self.uniform_hue(), saturation=self.uniform_sat())
            images = self.contrast(images, brightness=self.uniform_bri(), contrast=self.uniform_con())
...
class DaliDataset(object):
    ...
    def __init__(self, cfg, dataset_dir, image_sets, batch_size, training=False):
        ...
        self.device_ids  = torch.cuda.current_device() if len(cfg.DEVICE_ID) != 1 else cfg.DEVICE_ID[0] #",".join([str(d) for d in device_ids])
    ...
    def __iter__(self):
        for _ in range(self.__len__()):
            ...
            torch_targets = -1 * torch.ones([len(dali_boxes), max(max(num_detections),1), 5])
